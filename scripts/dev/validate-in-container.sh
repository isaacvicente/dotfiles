...
require_cmd "$CONTAINER_ENGINE" "Container engine not found in PATH: $CONTAINER_ENGINE"
...
log_info "Running bootstrap in clean container"
...
if command -v shellcheck >/dev/null 2>&1; then
  shellcheck scripts/*.sh scripts/dev/*.sh scripts/lib/*.sh
fi
...
echo "[OK] Container validation passed"
CONTAINER_SCRIPT

log_ok "Validation done"